import {SET_USER_ID, SET_USER_TOKEN} from "./actionTypes";

/**
 * This is an action creator, it creates an action and returns it
 * @param newUserIdParam The new user Id for the store to save
 * @returns {{type, newOrgId: *}} An action that updates the userId in the state
 */
export function setCurrentUserId(newUserIdParam) {
    return {type: SET_USER_ID, newUserId: newUserIdParam};
}
